...
def verify_skill():
    """Verify FastAPI skill meets standards."""
    ...
    if 'name: building-fastapi-apis' not in frontmatter:
    ...
    if code_blocks < 5:
        print(f"✗ Only {code_blocks} code examples. Need at least 5.")
        sys.exit(1)
    ...
    print(f"✓ building-fastapi-apis skill valid ({lines} lines, {len(reference_files)} reference files)")
    sys.exit(0)
