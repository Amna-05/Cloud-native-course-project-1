#!/usr/bin/env python3
"""Verify pytest skill SKILL.md structure and syntax."""
import os
import re
import sys
...
def verify_skill():
    """Verify pytest skill meets standards."""
    ...
    if 'name: testing-with-pytest' not in frontmatter:
    ...
    if code_blocks < 10:
        print(f"✗ Only {code_blocks} code examples. Need at least 10.")
        sys.exit(1)

    # Check for test patterns
    if '@pytest.fixture' not in body:
        print("✗ Missing @pytest.fixture examples")
        sys.exit(1)

    if 'def test_' not in body:
        print("✗ Missing test function examples")
        sys.exit(1)
    ...
    print(f"✓ testing-with-pytest skill valid ({lines} lines, {len(reference_files)} reference files)")
    sys.exit(0)
